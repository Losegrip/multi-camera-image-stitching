...
本工程的 seam 模块还预留了**接缝掩膜（seam mask）输入接口**，用于在拼接时显式避开关键目标。例如下面这组 road 场景：
...
- 左图：使用 **SAM (Segment Anything Model)** 对路牌、路灯、车道线等前景目标生成的二值掩膜；
- 中图：普通接缝优化，只考虑像素差/梯度等能量，在前景附近可能出现结构断裂；
- 右图：在能量图中对掩膜区域施加高惩罚，强制接缝绕开前景目标，保持路牌、路灯等结构完整。

在静态图像中，这个 seam mask 作为额外通道输入；在视频场景下，可以在首帧用 SAM 等模型生成掩膜并固化，后续帧复用同一掩膜与接缝，兼顾画质和实时性。这里本工程不对该环节做深入讲解，可以自行了解SAM分割模型。
